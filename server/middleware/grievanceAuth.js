    ...
    try {
        ...
        const grievanceRes = await pool.query(
            `SELECT 
             ...
             LEFT JOIN hostels h ON ga.hostel_id = h.hostel_id
        ...
        const isAssignedOfficer = userRoles.some(r => 
            ...
            r.hostel_id === grievance.hostel_id
        );

        // 3. Check if the user is a follower
        ...
        // 5. Attach grievance and follower status to request for downstream use
        req.grievance = grievance;
        req.isFollower = isFollower;
        next();
